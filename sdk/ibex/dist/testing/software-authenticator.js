...
function cborEncodeMap(map) {
    ...
    function cborEncodeHead(major, val) {
        const mt = major << 5;
        if (val < 24)
            return Buffer.from([mt | val]);
        if (val < 0x100)
            return Buffer.from([mt | 24, val]);
        if (val < 0x10000) {
...
export class SoftwareAuthenticator {
    ...
    async create(options) {
        ...
        const challenge = Buffer.from(options.challenge);
        const clientData = {
        ...
        const userId = options.user?.id
            ? toBase64Url(Buffer.from(options.user.id))
            : "";
    ...
    async get(options) {
        ...
        if (!cred) {
            throw new Error(`SoftwareAuthenticator: aucune credential trouvée pour rpId=${rpId}`);
        }
    ...
    findCredential(rpId, allowCredentials) {
        const allowed = allowCredentials?.map((c) => toBase64Url(Buffer.from(c.id)));
        const candidates = [];
        for (const [id, val] of Object.entries(this.credentials)) {
            if (!val || typeof val !== "object" || !("private_key" in val))
                continue;
            const s = val;
            if (s.rp_id !== rpId)
                continue;
    ...
    computePrfExtensionResults(options, stored) {
        const ext = options.extensions;
        const first = ext?.prf?.eval?.first;
        if (!first || !stored.prf_seed)
            return {};
        try {
 ...
 */
function buildPublicKeyCredential(input) {
    const rawId = input.rawId.buffer.slice(input.rawId.byteOffset, input.rawId.byteOffset + input.rawId.byteLength);
    let response;
